// DoorRECEIVER_SAFETY.ino - ESP32-C3 garage door receiver with optional safety permit relay
...
#define PERMIT_REPEAT_COUNT 3
#define PERMIT_REPEAT_DELAY_MS 10
#define PERMIT_LATE_BACKUP_DELAY_MS 30
#define PERMIT_LATE_BACKUP_ENABLED 1
...
struct SafetyPermitState {
  bool pending = false;
  uint16_t nonce = 0;
  uint8_t step = 0;
  uint32_t next_action_ms = 0;
  bool late_backup_sent = false;
};
...
static const bool SAFETY_SLAVE_ENABLED = true;
#else
static const bool SAFETY_SLAVE_ENABLED = false;
#endif
...
void sendPermit(uint8_t type, uint16_t nonce) {
#ifdef DOORSAFETY_SLAVE_MAC
  PermitMessage msg = {};
  msg.type = type;
...
void sendPermitBurst(uint8_t type, uint16_t nonce) {
  for (uint8_t i = 0; i < PERMIT_REPEAT_COUNT; i++) {
    feedWatchdog();
    sendPermit(type, nonce);
    delay(PERMIT_REPEAT_DELAY_MS);
  }
}
...
void scheduleSafetyPermit(uint16_t nonce) {
  safetyState.pending = true;
  safetyState.nonce = nonce;
  safetyState.step = 0;
  safetyState.next_action_ms = millis();
  safetyState.late_backup_sent = false;
}
...
void handleOpen(const SenderSecret &sc, const uint8_t *mac, const Message &msg) {
  ...
  if (SAFETY_SLAVE_ENABLED) {
    uint16_t nonce = (uint16_t)(esp_random() & 0xFFFF);
    scheduleSafetyPermit(nonce);
  } else {
    startRelayPulse();
  }
}
...
void processSafetyPermit() {
  if (!safetyState.pending) return;

  if (safetyState.step == 0) {
  ...
  if (safetyState.step == 3) {
    ...
    if (!safetyState.late_backup_sent && millis() >= safetyState.next_action_ms) {
      sendPermitBurst(PERMIT_TYPE_1, safetyState.nonce);
      sendPermitBurst(PERMIT_TYPE_2, safetyState.nonce);
      safetyState.late_backup_sent = true;
      safetyState.pending = false;
    }
#else
    safetyState.pending = false;
#endif
